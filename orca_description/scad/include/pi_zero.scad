...
function pin_pos_x(i) = - (19 * 1.27) + i * 2.54;
function pin_pos_y(j) = pi_hole_y - 1.27 + j * 2.54;
...
module pi_zero_body() {
  color("LimeGreen") difference() {
    ...
    // GPIO header holes
    for (i = [0:19], j = [0, 1]) {
      pcb_hole(pin_pos_x(i), pin_pos_y(j), 0.6, pi_z);
    }
  ...
  pcb_component(hdmi, - 20.1, - 11.4, yaw = 90);
